...
export class HealthRecordsService {
  ...
  async upload(
    ...
    if (dto.recordType === HealthRecordType.LAB_TEST && !dto.testName) {
      throw new BadRequestException(
        'testName is required for LAB_TEST records',
      );
    }
    if (dto.recordType === HealthRecordType.MEDICATION && !dto.drug) {
      throw new BadRequestException('drug is required for MEDICATION records');
    }
    if (
      dto.recordType === HealthRecordType.ALLERGY &&
      (!dto.allergyType || !dto.cause)
    ) {
      throw new BadRequestException(
        'allergyType and cause are required for ALLERGY records',
      );
    }
      ...
      files.length > 0
        ? await Promise.all(
            files.map(async (file) => {
              ...
              const { fileUrl, fileUrlExpiresAt } =
                await this.s3.getDownloadUrl(key);
              return {
  ...
  async findAll(userId: string, query: HealthRecordsQueryDto) {
    ...
    const sortColumn =
      sortBy === HealthRecordSortBy.RECORD_DATE
    ...
    const [rows, [{ total }]] = await Promise.all([
      ...
      this.db.select({ total: count() }).from(healthRecords).where(where),
    ]);

    const now = Date.now();
    const refreshed = await Promise.all(
      rows.map((r) => this.refreshFiles(r, now)),
    );
  ...
  async remove(userId: string, id: string) {
    ...
    await this.dashboard.invalidate(userId);
    this.activityLog.log(userId, 'HEALTH_RECORD_DELETED', {
      recordId: id,
      title: record.title,
    });
  }
