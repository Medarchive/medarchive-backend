...
export const zkProofStatusEnum = pgEnum('zk_proof_status', ['PENDING', 'GENERATED', 'FAILED']);

export const healthRecordProofs = pgTable('health_record_proofs', {
  id: uuid('id').primaryKey().default(sql`uuidv7()`),
  healthRecordId: uuid('health_record_id').notNull().unique().references(() => healthRecords.id, { onDelete: 'cascade' }),
...
export const healthRecordProofsRelations = relations(healthRecordProofs, ({ one }) => ({
