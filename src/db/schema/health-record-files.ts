...
export const healthRecordFiles = pgTable(
  'health_record_files',
  {
    ...
    fileUrlExpiresAt: timestamp('file_url_expires_at', {
      withTimezone: true,
    }).notNull(),
  ...
  (t) => [index('health_record_files_record_id_idx').on(t.healthRecordId)],
);

export const healthRecordFilesRelations = relations(
  healthRecordFiles,
  ({ one }) => ({
